...
| R4 | SMUX 收端多锁合并 `StreamInner` | ✅ 已完成 | `RecvInner`+`SendInner`；删 legacy `recv_buf`；见 docs + smux AGENTS |
...
> **设计定稿（未改代码）：** 见 [`docs/R4_STREAM_INNER_AND_COMMON_EXTRACT.md`](docs/R4_STREAM_INNER_AND_COMMON_EXTRACT.md)  
> — 落地为 `RecvInner` + `SendInner` 双 half（先 recv 后 send）、删除 legacy `recv_buf`、放锁再 wake；同文档含 common 二期（QPPPort/pipe/snmp）与 e2e/stress/bench 门禁。
