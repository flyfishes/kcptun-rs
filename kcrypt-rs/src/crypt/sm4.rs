...
//!
//! Uses CFB-128 mode (cfb16) matching Go's `encrypt16`/`decrypt16`.
//!
//! # Important
//!
...
impl Sm4Crypt {
    pub fn new(key: &[u8]) -> Self {
        ...
        for i in 0..4 {
            k[i] = u32::from_be_bytes(padded[i * 4..i * 4 + 4].try_into().unwrap()) ^ FK[i];
        }
        let mut rk = [0u32; 32];
        for i in 0..32 {
    ...
    fn encrypt_block(&self, inp: &[u8; 16], out: &mut [u8; 16]) {
        let mut x = [0u32; 36];
        for i in 0..4 {
            x[i] = u32::from_be_bytes(inp[i * 4..i * 4 + 4].try_into().unwrap());
        }
        for i in 0..32 {
            x[i + 4] = x[i] ^ t(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ self.rk[i]);
        }
        // Output in reverse order (X35, X34, X33, X32)
        for i in 0..4 {
            let v = x[35 - i].to_be_bytes();
...
mod tests {
    ...
    #[test]
    fn sm40() {
        rt(&Sm4Crypt::new(&[0u8; 16]), &mut b"hello kcp sm4!".to_vec());
    }
}
